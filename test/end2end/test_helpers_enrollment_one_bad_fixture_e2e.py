...
_FIXTURE_EVENTS_FILE = (
	...
	/ "fixtures"
	/ "events"
	/ "enrollment_events_one_bad.json"
)


@pytest.mark.e2e
def test_enrollment_fixture_one_bad_entity_writes_to_neo4j(tmp_path):
	"""Fixture-driven e2e: one entity canonical, one entity faulty.

	This validates that the pipeline faithfully persists declared causality as-is
	(no repair, no inference) while still being self-consistent.
	"""
	...
	try:
		with driver.session() as session:
			# Ensure both entities are present.
			entity_ids = [
				rec["entity_id"]
			...
			# enrollment-2 must NOT match canonical edges (it is deliberately faulty)
			types_2 = session.run(
				"""
			...
			assert set(types_2) == canonical_types_set
			...
			assert {tuple(p) for p in edges_2} != canonical_edges_set
	finally:
		driver.close()
